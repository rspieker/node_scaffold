...
function DOMSerializer()
{
	...
	{
		return !(isHTML(format) && pattern.quoteLessAttribute.test(value));
	}
	...
	function serializeToString(node, buffer, options)
	{
		...
		switch(node.nodeType)
		{
			...
			case serializer.ATTRIBUTE_NODE:
				...
				else if (!quoteAttribute(node.value, options.format))
					return buffer.push(' ', node.name, '=', node.value);
				return buffer.push(' ', node.name, '="', (node.value || node.name), '"');
			...
			case serializer.DOCUMENT_FRAGMENT_NODE:
				...
				if (!options.format)
				{
					if (serializer.DOCUMENT_TYPE_NODE === child.nodeType)
						options.format = detectFormat(child.publicId || child.systemId || child.name);
					else if (serializer.PROCESSING_INSTRUCTION_NODE === child.nodeType)
						options.format = detectFormat(child.target || child.data);
				}
	 ...
	 *  @return string
	 */
	serializer.toString = function(dom, options)
	{
		var buffer = [];
...
{
	return new DOMSerializer().toString(dom, options);
};