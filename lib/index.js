...
function Template(source, done, parent)
{
	...
	function clean()
	{
		...
		//  normalize the document, so adjacent text nodes get combined (as the next step will try to clean them up individually)
		dom.normalize();


/*
		//  This can all be taken care of by the serializer now...


		//  remove comments from the output, unless there is some sort of copyright notice in it
		xpath('//comment()', function(node){
			if (!/copyright/i.test(node.nodeValue))
				node.parentNode.removeChild(node);
		});

		//  remove all whitespace only text nodes in <html> but not in <body> or <script>
		...
		//  truncate all whitespace in text nodes in <body> but not in <script>
...
*/
	...
	function render(asDOM)
	{
		...
		return asDOM ? dom : DOMSerializer(dom, {
			format: 'html',
